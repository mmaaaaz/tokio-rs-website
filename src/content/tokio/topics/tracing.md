...
information about _temporality_ and _causality_ — unlike a log message, a
...
provides the complementary concept of _events_. Both [`Span`]s and [`Event`]s
...
3. has some structured data associated with it.
   - `fields(...)` indicates that emitted span _should_ include
     the `fmt::Display` representation of the connection's `SocketAddr` in a field
     called `peer_addr`.
   - `skip(self)` indicates that emitted span should _not_ record `Handler`'s debug representation.
